...
struct AudioDriverMock : audio::drivers::AudioDriver {
    MOCK_METHOD3(createAsyncTrack, std::optional<audio::drivers::TrackHandle>(
        const audio::drivers::AConfiguration&, const audio::base::AudioBuffer<>&, const audio::drivers::TrackBufferReadyCallback));
    MOCK_METHOD1(releaseTrack, void(const audio::drivers::TrackHandle&));
    MOCK_CONST_METHOD1(start, bool(const audio::drivers::TrackHandle&));
    MOCK_METHOD1(resume, bool(const audio::drivers::TrackHandle&));
